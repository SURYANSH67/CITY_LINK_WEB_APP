...
class User(db.Model, UserMixin):
    ...
    role = db.Column(db.String(50), nullable=False, default='Citizen') # Change default to 'Admin' # e.g., 'Admin', 'Official'
...
class Complaint(db.Model):
    ...
    reporter_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=True) # Can be null for anonymous reports

    # This relationship lets us do 'complaint.reporter' to get the User object.
    reporter = db.relationship('User', foreign_keys=[reporter_id], backref='reported_complaints')
    
    # The 'assigned_to' relationship remains the same.
    assigned_to_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=True)
    assigned_to = db.relationship('User', foreign_keys=[assigned_to_id], backref='assigned_complaints')

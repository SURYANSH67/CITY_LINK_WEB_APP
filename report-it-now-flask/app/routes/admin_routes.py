...
def dashboard():
    ...
    status_counts_query = base_query.with_entities(Complaint.status, func.count(Complaint.status)).group_by(Complaint.status).all()
    ...
    issue_counts_query = base_query.with_entities(Complaint.issue_type, func.count(Complaint.issue_type)).group_by(Complaint.issue_type).order_by(func.count(Complaint.issue_type).desc()).all()
    ...
    if search_term:
        final_query = final_query.filter(or_(Complaint.ticket_id.ilike(f'%{search_term}%'), Complaint.address_text.ilike(f'%{search_term}%')))
    if status_filter:
        final_query = final_query.filter(Complaint.status == ComplaintStatus[status_filter])
    if issue_type_filter:
        final_query = final_query.filter(Complaint.issue_type == IssueType[issue_type_filter])
...
def complaint_detail(complaint_id):
    ...
    if current_user.role == 'Admin':
        ...
        form.assigned_to.choices = [(0, '--- Unassigned ---')] + [(o.id, o.name) for o in officials]
    else:
...
def api_complaints():
    ...
    if search_term:
        query = query.filter(or_(Complaint.ticket_id.ilike(f'%{search_term}%'), Complaint.address_text.ilike(f'%{search_term}%')))
    if status_filter:
